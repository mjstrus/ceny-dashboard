...
def get_price_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    display_cols = ['ID', 'Nazwa', 'Typ_Pełny', 'Cena_Stara', 'Miał_Rabat_10%', 
                    'Cena_Faktyczna', 'Cena_Docelowa', 'Wzrost_Kwota', 
                    'Wzrost_%_Od_Faktycznej', 'Wzrost_%_Od_Cennika']
    
    df_display = df[display_cols].copy()
    df_display.columns = ['ID', 'Nazwa', 'Typ', 'Cennik', 'Rabat?', 
                          'Płacili', 'Będą płacić', 'Wzrost PLN', 'Wzrost %', 'Wzrost Cennika %']
    
    # Format
    df_display['Rabat?'] = df_display['Rabat?'].map({1: '✓ TAK (-10%)', 0: '✗ Nie'})
