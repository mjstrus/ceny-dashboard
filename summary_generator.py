...
def generate_summary(df: pd.DataFrame) -> dict:
    ...
    if 'Status' in df.columns:
        ...
        czerwony = len(df[df['Status'] == 'Czerwony'])  # >20% ale mieli rabat
        czarny = len(df[df['Status'] == 'Czarny'])  # >20% bez rabatu = RYZYKO!
        
        # CZARNY: ile miało rabat vs ile nie miało (ale CZARNY = zawsze bez rabatu!)
        czarny_bez_rabatu = czarny  # Wszystkie CZARNY mają bez rabatu
        czarny_z_rabatem = 0  # CZARNY to TYLKO ci bez rabatu
    else:
        # Fallback
        ...
        czerwony = 0
        czarny = len(paying_df[paying_df['Wzrost_%_Od_Faktycznej'] > 20])
        czarny_z_rabatem = 0
        czarny_bez_rabatu = czarny
    ...
    summary = {
        ...
        'Czarny_Cnt': czarny,
        'Czarny_Z_Rabatem_Cnt': czarny_z_rabatem,
        'Czarny_Bez_Rabatu_Cnt': czarny_bez_rabatu,
        
        'Rabat_CZARNY_PLN': 20.0,  # Rabat 20 PLN netto dla CZARNYCH klientów (oba warunki)
...
def get_alerts(summary: dict) -> list:
    ...
    # Alert: CZARNI klienci (rzeczywisty wzrost >20%, BEZ rabatu = RZECZYWISTE RYZYKO!)
    if summary.get('Czarny_Cnt', 0) > 0:
        pct_black = (summary.get('Czarny_Cnt', 0) / (summary.get('Liczba_Klientów_Standard', 0) + summary.get('Liczba_Klientów_VIP', 0))) * 100 if (summary.get('Liczba_Klientów_Standard', 0) + summary.get('Liczba_Klientów_VIP', 0)) > 0 else 0
        alerts.append(f"⚫ {summary.get('Czarny_Cnt', 0)} klientów ({pct_black:.1f}%) ma wzrost >20% (bez rabatu) — WYSOKIE RYZYKO!")
        alerts.append(f"💡 Warto im zaproponować rabat 20 PLN netto za oba warunki:")
        alerts.append(f"   • 10 PLN rabatu za dostarczenie dokumentów do 3. dnia miesiąca")
        alerts.append(f"   • 10 PLN rabatu za płatność faktury w 3 dni")
        alerts.append(f"   • Wniosek o wakacje składkowe w cenie")
    
    # Alert: CZERWONI klienci (wzrost >20% ale mieli rabat = uzasadnione)
    if summary.get('Czerwony_Cnt', 0) > 0:
        ...
        alerts.append(f"🔴 {summary.get('Czerwony_Cnt', 0)} klientów ({pct_red:.1f}%) ma wzrost >20% ale mieli rabat — łatwo wyjaśnić")
        alerts.append(f"💡 Wyjaśnić że wzrost wynika z anulowania rabatu za Saldeo, nie podwyżki ceny")
    
    # Alert: ŻÓŁCI klienci (wzrost 10-20%)
    if summary.get('Zolty_Cnt', 0) > 0:
        ...
        alerts.append(f"🟡 {summary.get('Zolty_Cnt', 0)} klientów ({pct_yellow:.1f}%) ma wzrost 10-20% — wymaga komunikacji")
